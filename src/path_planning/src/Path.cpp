...
void Path::calculate(Pos start, Grid& g)
{
	calculate(start.x, start.y, g);
}

void Path::calculate(int startX, int startY, Grid& g)
{
	...
	while (!m_frontier.empty())
	{
		...
		for (std::shared_ptr<Location> neighbor : currentLocation->paths)
		{
			...
			if (neighbor != startLocation && (newCostSoFar < neighbor->costSoFar || neighbor->costSoFar == 0.0))
			{
				...
				if (lineOfSight(g, neighbor, currentLocation->from)) {
					neighbor->from = currentLocation->from;
					//neighbor->costSoFar = currentLocation->from->costSoFar + calculateCostBetween(currentLocation->from, neighbor);
				}
...
bool Path::lineOfSight(Grid& g, std::shared_ptr<Location> l1, std::shared_ptr<Location> l2)
{
	if (l1 == nullptr || l2 == nullptr) {
		return false;
	}

	return g.lineOfSight({ l1->x, l1->y }, { l2->x, l2->y }, 0.5f);
}
